...
namespace BanMayBay
{
    public partial class frmGame3 : Form
    {
        ...
        private void MoveTien(int vTien)
        {
            if (picTien.Top > this.Height)
            {
                picTien.Visible = true;
                int x = rdDa.Next(100, 350);
                picTien.Location = new Point(x, 0);
            }
            picTien.Top += vTien;
        }

        private void frmGame3_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            if (TienHa.Coint + TienHa.Pass >= 100)
            {
